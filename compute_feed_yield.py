    ...
    for feed in feed_item_list:
        ...
        if feed not in FBS_to_SUA_df.index:
            ...
            if np.any(feed_demand_df.loc[(feed_demand_df['Country']==country),'Item Code'].isin([feed])):
                feed_tot_demand+=feed_demand_df.loc[(feed_demand_df['Item Code']==feed) & (feed_demand_df['Country']==country),'Value'].values[0]
                yields_df.loc[0,country]+=feed_demand_df.loc[(feed_demand_df['Item Code']==feed) & (feed_demand_df['Country']==country),'Value'].values[0]*feed_yield*FM_to_DM
                # if country=="Ireland":
