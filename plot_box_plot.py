...
#    print("Plot production with different metrics...")
    df_to_concat=pd.concat([activity_df[['Country','Intensification',production+' 2010',production+' index',production,"Allocation rule"]],pd.DataFrame([production]*len(activity_df['Country']),columns=['Item'])],axis=1,sort=True)
...
table = pd.pivot_table(df_with_all_prod_to_plot, values=['Production in 2010 (Mt)','Production in 2050 (Mt)'], index=['Country','Item'],columns=["Allocation rule","Intensification"],aggfunc=np.mean)
table.index.name=None
...
df_to_print=pd.DataFrame(columns=['Yield','Yield 2010','Yield index (relative to 2010)','Country','Item','Production','Intensification'])
...
df_with_all_prod_to_plot=pd.DataFrame(columns=['UP','UP index','Allocation rule','UP 2010','Country','Item',"Intensification"])
for item in item_list:
    activity_df['UP index '+item]=activity_df['UP '+item].values/activity_df['UP 2010 '+item].values
    df_to_concat=pd.concat([activity_df[['Country','UP 2010 '+item,'UP index '+item,'UP '+item,"Allocation rule","Intensification"]],pd.DataFrame([item]*len(activity_df['Country']),columns=['Item'])],axis=1,sort=True)
...
for item in ['Grass','Feed','Rice','Total']:
    df_to_concat=pd.concat([activity_df[['Country','National '+item+' area index','National '+item+' area 2010','National '+item+' area',"Allocation rule","Intensification"]],pd.DataFrame([item]*len(activity_df['Country']),columns=['Item'])],axis=1,sort=True)
...
plt.close("all")
table = pd.pivot_table(df_to_plot, values=['Area in 2010 (Mha)','Area in 2050 (Mha)'], index=['Country','Item'],columns=["Allocation rule","Intensification"],aggfunc=np.mean)
table.index.name=None
...
df_to_plot=pd.DataFrame(columns=['N2O emissions','Allocation rule','Country','Item',"Intensification"])
for item in ['manure','fert']:
    df_to_concat=pd.concat([activity_df[['Country','N2O '+item+' index', 'N2O '+item+' 2010','N2O '+item,"Allocation rule","Intensification"]],pd.DataFrame([item]*len(activity_df['Country']),columns=['Item'])],axis=1,sort=True)
...
df_to_plot=pd.DataFrame(columns=['CO2 offset','Allocation rule','Country','Item',"Intensification"])
for item in ['Grass','Feed','Rice','Total']:
    df_to_concat=pd.concat([activity_df[['Country','Intensification',item+' offset',"Allocation rule"]],pd.DataFrame([item]*len(activity_df['Country']),columns=['Item'])],axis=1,sort=True)
...
for gaz in ["CO2","CH4","N2O","AFOLU","Import"]:
    ...
    df_tmp['Intensification']=activity_df.loc[:,"Intensification"].values
    # df_tmp['Mitigation']=activity_df.loc[:,"Mitigation"].values
    df_to_concat=pd.concat([df_tmp[['Country','GWP (MtCO2eq)',"Allocation rule","Intensification"]],pd.DataFrame([gaz]*len(df_tmp['Country']),columns=['Item'])],axis=1,sort=True)
...
plt.close("all")
table = pd.pivot_table(df_to_plot, values=['GWP (MtCO2eq)'], index=['Country','Item'],columns=["Allocation rule","Intensification"],aggfunc=np.mean)
table.index.name=None
