...
for suffix_variant in ["_carbon+25","_carbon-25","_yield+25","_mitigation-25",""]:
    ...
    variant_df=pd.DataFrame([suffix_variant_name.replace("_","")]*len(df_tmp.loc[df_tmp["Intensification"]=="Base","Country"]),columns=['Variant'])
    variant_df.index=df_tmp.loc[df_tmp["Intensification"]=="Base","AFOLU balance (with eGWP*)"].index
    df_to_concat=pd.concat([df_tmp.loc[df_tmp["Intensification"]=="Base",["AFOLU balance (with eGWP*)","Total production","Country","Allocation rule"]],variant_df],axis=1,sort=True)
...
df_to_table_median=df_all_variants.pivot_table(values=['AFOLU balance (with eGWP*)',"Total production"],columns=["Variant"],index=["Country","Allocation rule"],aggfunc=np.median)
df_to_table_std=df_all_variants.pivot_table(values=['AFOLU balance (with eGWP*)',"Total production"],columns=["Variant"],index=["Country","Allocation rule"],aggfunc=np.std)
