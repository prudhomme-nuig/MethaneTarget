...
df_all_variants=pd.DataFrame(columns=["Country","AFOLU balance","Allocation rule","Variant"])
for suffix_variant in ["_carbon+50","_carbon-50","_yield+50","_yield-50","_mitigation-50","_no_mitigation",""]:
    ...
    df_to_concat=pd.concat([df_tmp["Country"],df_tmp["AFOLU balance"],df_tmp["Allocation rule"],pd.DataFrame([suffix_variant_name.replace("_","")]*len(df_tmp["Total production"]),columns=['Variant'])],axis=1,sort=True)
    df_all_variants=pd.concat([df_all_variants,df_to_concat],axis=0,sort=True)
...
df_to_table=df_all_variants.pivot_table(values='AFOLU balance',columns=["Variant"],index=["Country","Allocation rule"],aggfunc=np.median)
df_to_table.to_excel("output/sensitivity_analysis_median.xlsx",index_label=None,float_format = "%0.3f")
df_for_test=df_all_variants.pivot(values=['AFOLU balance',"Allocation rule","Country"],columns="Variant")
df_result={}
for country in country_list:
    ...
    for rule in rule_list:
        ...
        for group in ["carbon","yield","mitigation"]:
            ...
            df=df_for_test["AFOLU balance"][group_list][country_rule_mask]
            df=df.astype("float")
